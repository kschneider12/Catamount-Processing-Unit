...
_CC_MAP = {"BEQ": 0b00, "BNE": 0b01, "BLT": 0b10, "BGE": 0b11}
...
def _imm(token, bits):
    ...
    Values are silently masked to `bits` wide, so out-of-range values
    (e.g. #257 for an 8-bit field) lose their high bits without error.
    This is intentional: it matches the behavior of most real assemblers
    and lets programmers use negative literals like #-1 naturally.
    """
    if token.startswith("#"):
        token = token[1:]
    value = int(token, 0)
...
def assemble(lines):
    ...
    for line in cleaned:
        ...
        if mnemonic == "MOV":
            ...
            opcode = ISA["ADDI"]["opcode"]
            word = (opcode << 12) | (rd << 9) | (ra << 6)
